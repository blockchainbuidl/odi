...
contract ETHStaking is Ownable {
    ...
    // The block number when ODI mining ends.
    ...
    constructor(
        ...
        // staking pool
        poolInfo.push(PoolInfo({lpToken: _lp, allocPoint: 1000, lastRewardBlock: startBlock, accODIPerShare: 0}));
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 ODIReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function deposit() public payable {
        ...
        if (msg.value > 0) {
            ...
            assert(IWKLAY(WKLAY).transfer(address(this), msg.value));
